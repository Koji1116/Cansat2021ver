...
def initTSL2572():
    ...
    setTSL2572reg(TSL2572_COMMAND | TSL2572_TYPE_INC |
                  TSL2572_ENABLE, TSL2572_AEN | TSL2572_PON)
    return 0
...
def getTSL2572reg(reg):
    dat = i2c.read_i2c_block_data(
        TSL2572_ADR, TSL2572_COMMAND | TSL2572_TYPE_INC | reg, 1)
    return dat


def getTSL2572adc():
    dat = i2c.read_i2c_block_data(
        TSL2572_ADR, TSL2572_COMMAND | TSL2572_TYPE_INC | TSL2572_C0DATA, 4)
    adc0 = (dat[1] << 8) | dat[0]
    adc1 = (dat[3] << 8) | dat[2]
    return[adc0, adc1]
...
if __name__ == '__main__':
    ...
    while 1:
        ...
        if ((lux1 <= 0) and (lux2 <= 0)):
            ...
            Other.saveLog('LuxTest', datetime.datetime.now(),
                          startTime - time.time(), lux1)
        elif (lux1 < lux2):
            print("{:.1f} Lx".format(lux2))
            Other.saveLog('LuxTest', datetime.datetime.now(),
                          startTime - time.time(), lux2)
        time.sleep(0.2)
