...
wb = openpyxl.load_workbook('GPS_walk.xlsx')
...
d=139.903383
s=6
for i in range(10,506):
    a =sheet.cell(row=i, column=5).value
    b =sheet.cell(row=i, column=6).value
    if a != 0 and b !=0 and c != 0 and d !=0 and a != None and b != None and c != None and d != None :
        c =sheet.cell(row=i-s, column=5).value
        d =sheet.cell(row=i-s, column=6).value
        if  c != None and d != None:
            
            data=vincenty_inverse(a, b,c,d)
        else:
            c =sheet.cell(row=i-s-1, column=5).value
            d =sheet.cell(row=i-s-1, column=6).value
            data=vincenty_inverse(a, b,c,d)
        e =+ data['distance']
        print(data['distance'])
        n =+ 1
